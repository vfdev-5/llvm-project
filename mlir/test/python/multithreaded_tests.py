...
class TestAllMultiThreaded(unittest.TestCase):
    ...
    def test_check_pyoperation_race(self):
        # Checks the race when one thread creates PyOperation python handle,
        # do some work (fetch op name) and finally decreases the refcount of
        # mod.operation which can lead to the object destruction if the
        # reference was not yet increased.
        # At the same time, another thread finds a PyOperation in
        # liveOperations and increases the refcount, but it is too late.
        num_workers = 40
        num_runs = 20
